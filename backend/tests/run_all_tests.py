...
def run_all_tests():
    ...
    # ========================================================================
    # 14. API тесты - test_save_notifications.py
    # ========================================================================
    print_test_file(
        "tests/api/test_save_notifications.py",
        "Тестирование сохранения уведомлений"
    )
    try:
        import subprocess
        result = subprocess.run(
            [sys.executable, os.path.join(os.path.dirname(__file__), "api/test_save_notifications.py")],
            capture_output=True,
            text=True
        )
        print(result.stdout)
        if result.stderr:
            print(result.stderr)
        success = result.returncode == 0
        results.append(("api/test_save_notifications.py - Сохранение уведомлений", success))
    except Exception as e:
        print(f"❌ Ошибка: {e}")
        import traceback
        traceback.print_exc()
