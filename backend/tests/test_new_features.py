...
def test_analytics():
    ...
    try:
        ...
        for i, master in enumerate(kpi['masters']['top_masters'], 1):
            print(f"   {i}. {master['master']} - {master['revenue']} AED ({master['count']} записей)")

        print(f"\n✅ Топ-5 услуг:")
        for i, service in enumerate(kpi['services']['top_services'], 1):
            print(f"   {i}. {service['service']} - {service['revenue']} AED ({service['count']}x)")

        # Тест 1.2: Тренды
        print_subsection("Сравнение с предыдущим периодом")
        if kpi['trends']:
            print(f"   Изменение дохода: {kpi['trends']['revenue_change']:+.1f}%")
            print(f"   Изменение записей: {kpi['trends']['bookings_change']:+.1f}%")
            print(f"   Изменение клиентов: {kpi['trends']['clients_change']:+.1f}%")
