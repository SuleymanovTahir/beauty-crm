"""
Миграция: Добавление Soft Delete (Консолидированная)
"""
...
def run_migration():
    ...
    try:
        log_info("🔧 Начало миграции: Soft Delete", "migration")
        
        # 1. Добавляем deleted_at к bookings
        c.execute("ALTER TABLE bookings ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP NULL")
        
        # 2. Добавляем deleted_at к clients
        c.execute("ALTER TABLE clients ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP NULL")
        
        # 3. Добавляем deleted_at к users
        c.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP NULL")
        
        # 4. Создаем индексы для быстрого поиска
        c.execute("CREATE INDEX IF NOT EXISTS idx_bookings_deleted ON bookings(deleted_at) WHERE deleted_at IS NULL")
        c.execute("CREATE INDEX IF NOT EXISTS idx_clients_deleted ON clients(deleted_at) WHERE deleted_at IS NULL")
        c.execute("CREATE INDEX IF NOT EXISTS idx_users_deleted ON users(deleted_at) WHERE deleted_at IS NULL")
