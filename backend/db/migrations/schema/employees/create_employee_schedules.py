...
def create_employee_schedules():
    ...
    # Проверяем количество мастеров
    c.execute("SELECT COUNT(*) FROM employees WHERE is_active = 1")
    employees_count = c.fetchone()[0]

    conn.close()

    # Если есть мастера, но нет расписаний - это ошибка!
    if employees_count > 0 and count == 0:
        print(f"❌ ОШИБКА: Есть {employees_count} мастеров, но нет расписаний!")
        return False

    # Если мастеров нет - это нормально (они создадутся позже в DATA миграциях)
    # Если мастера есть и расписания есть - отлично!
    return True
