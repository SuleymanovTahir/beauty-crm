...
def recreate_database():
    ...
    db_user = os.getenv('POSTGRES_USER', 'postgres') # Для удаления/создания БД лучше использовать суперюзера или владельца
    db_password = os.getenv('POSTGRES_PASSWORD', '')
    db_host = os.getenv('POSTGRES_HOST', 'localhost')
    db_port = os.getenv('POSTGRES_PORT', '5432')
    
    # ВАЖНО: Для операций DROP/CREATE DATABASE часто нужны права суперпользователя (postgres)
    # Если beauty_crm_user не имеет прав CREATEDB, скрипт упадет.
    # Поэтому попробуем сначала использовать пользователя из конфига, 
    # но если это обычный юзер, возможно придется переключиться на системного.
    
    print(f"⚙️  Параметры подключения: host={db_host}, user={db_user}, db={db_name}")
