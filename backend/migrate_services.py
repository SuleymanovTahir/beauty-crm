...
SERVICES_DATA = [
    ...
    },
    {
        'key': 'bikini_line',
        'name': 'Bikini Line',
        'name_ru': 'Классическое бикини',
        'name_ar': 'خط البكيني',
        'price': 100,
        'currency': 'AED',
        'category': 'Waxing',
        'description': 'Basic bikini line waxing',
        'description_ru': 'Базовая эпиляция линии бикини',
        'benefits': ['Quick', 'Comfortable', 'Clean look']
    },
    {
        'key': 'full_legs',
        'name': 'Full Legs',
...
def migrate_services():
    ...
    try:
        ...
        if existing_count > 0:
            ...
            response = input("   Удалить существующие и загрузить заново? (yes/no): ")
            if response.lower() not in ['yes', 'y']:
                print("❌ Миграция отменена")
                conn.close()
                return 0
            
            # Удалить существующие услуги
            c.execute("DELETE FROM services")
            print("✅ Существующие услуги удалены")
    ...
    print(f"\n📥 Загрузка {len(SERVICES_DATA)} услуг...")
    print()
    ...
    for service in SERVICES_DATA:
        try:
            ...
            c.execute("""INSERT INTO services 
                        (service_key, name, name_ru, name_ar, price, currency, 
                         category, description, description_ru, benefits, 
            ...
            success_count += 1
            print(f"✅ {service['name_ru']} ({service['price']} {service['currency']})")
            
        except sqlite3.IntegrityError as e:
            error_count += 1
            print(f"❌ Дубликат: {service['key']}")
