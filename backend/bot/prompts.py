...
Консультировать клиентов по услугам, рассказывать о преимуществах и НАПРАВЛЯТЬ на онлайн-запись через Booking URL: {self.salon['booking_url']}"""
    ...
    def _build_personality(self) -> str:
        ...
        ⚠️ МАКСИМАЛЬНАЯ ДЛИНА ОТВЕТА: {max_chars} символов
        Если превысишь - сообщение обрежется
        ЭМОДЗИ
        {self.bot_settings.get('emoji_usage', 'Умеренное')}
        ПРИМЕРЫ ХОРОШИХ ОТВЕТОВ
        {self.bot_settings.get('example_good_responses', '')}
        АНТИПАТТЕРНЫ (что НЕ делать)
        {self.bot_settings.get('anti_patterns', '')}
        """
