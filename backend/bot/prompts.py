...
8. **ПРОВЕРЯЙ ФАКТЫ - НЕ СОГЛАШАЙСЯ СО ВСЕМ:**
   ...
   ⚠️ КОГДА КЛИЕНТ СПРАШИВАЕТ "КТО МАСТЕР?" ИЛИ "А КТО МАСТЕР?":
   Клиент хочет понять СТОИТ ЛИ к нему идти! Не просто назови имя!
   
   ✅ ПРАВИЛЬНО:
   "Мастер Гуля - наш специалист по волосам с опытом 5+ лет. Она делает отличные кератиновые выпрямления!"
...
- "Есть Wi-Fi?" → Скажи да или нет"""
    ...
    def _build_masters_list(self, client_language: str = 'ru') -> str:
        ...
        for emp in employees[:5]:
            ...
            # ✅ Для русского языка используем оригинальное имя, для других - транслитерацию
            if client_language == 'ru':
                emp_name_display = str(emp_name) if emp_name else "Мастер"
            else:
            ...
            # ✅ Получаем опыт и био мастера
            experience = full_emp[15] if len(full_emp) > 15 else None  # experience field
            bio = full_emp[16] if len(full_emp) > 16 else None  # bio field
