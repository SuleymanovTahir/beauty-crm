...
def create_test_users():
    ...
    test_users = [
        ...
        },
        {
            ...
            "role": "admin",
            "position": "Администратор салона",
            "bio": "Управление записями и клиентами",
        },
    ...
    print(f"\n🔐 Все пользователи имеют пароль: {test_password}")
    print(f"\n📧 Все email подтверждены\n")

    # Показываем список созданных пользователей по ролям
    print("=" * 60)
    print("📋 СПИСОК ТЕСТОВЫХ ПОЛЬЗОВАТЕЛЕЙ")
    print("=" * 60)

    roles_order = ["director", "admin", "manager", "sales", "marketer", "employee"]
    role_names = {
        "director": "👑 Директора",
    ...
    for role in roles_order:
        ...
        if role_users:
            print(f"\n{role_names[role]}:")
            for u in role_users:
                print(f"   • {u['username']:12} | {u['full_name']:20} | {u['email']}")
