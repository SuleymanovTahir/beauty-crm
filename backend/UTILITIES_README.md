# Утилиты для работы с базой данных

Этот документ описывает утилиты для работы с базой данных Beauty CRM.

## 📋 Список утилит

### 1. fill_bot_settings.py
**Назначение:** Заполнение и проверка настроек бота

**Использование:**
```bash
# Проверить текущее состояние полей
python fill_bot_settings.py --check

# Заполнить только пустые поля (по умолчанию)
python fill_bot_settings.py

# Перезаписать все поля
python fill_bot_settings.py --force
```

**Заполняемые поля:**
- price_explanation - объяснение ценообразования
- objection_handling - обработка возражений
- negative_handling - работа с недовольными клиентами
- example_dialogues - примеры диалогов
- context_memory - контекстная память
- avoid_repetition - избегание повторений
- conversation_flow_rules - правила ведения диалога
- personality_adaptations - адаптация под клиента
- smart_objection_detection - распознавание возражений

---

### 2. check_db.py
**Назначение:** Комплексная проверка состояния базы данных

**Использование:**
```bash
python check_db.py
```

**Что проверяет:**
- ✅ Пользователи (наличие email, дней рождения)
- ✅ Сотрудники (контактная информация)
- ✅ Настройки бота (9 критических полей)
- ✅ Наличие всех таблиц
- ✅ Количество записей в каждой таблице

**Пример вывода:**
```
================================================================================
🔍 ПРОВЕРКА БАЗЫ ДАННЫХ
================================================================================

📋 ПОЛЬЗОВАТЕЛИ:
Всего пользователей: 8
С датой рождения: 1
⚠️  Пользователей БЕЗ email: 4

🤖 НАСТРОЙКИ БОТА:
Заполнено: 9/9
✅ Все критические поля заполнены

📊 ТАБЛИЦЫ:
Найдено таблиц: 10/10
```

---

### 3. setup_test_notifications.py
**Назначение:** Создание тестовых данных для проверки уведомлений

**Использование:**
```bash
# Создать тестовые события на завтра
python setup_test_notifications.py --email your@email.com

# Создать тестовые события через 3 дня
python setup_test_notifications.py --email your@email.com --days 3
```

**Что создает:**
- 👤 Тестовый клиент с днем рождения
- 👥 Тестовый сотрудник с днем рождения
- 📅 Тестовые записи (2 шт)
- ⚙️ Настройки напоминаний о записях
  - За 24 часа
  - За 2 часа

**После создания:**
1. Проверьте почту указанного email
2. Запустите планировщик дней рождения:
   ```bash
   python -m scheduler.birthday_checker
   ```
3. Запустите планировщик напоминаний:
   ```bash
   python -m scheduler.booking_reminder_checker
   ```

---

## 🔄 Типичный workflow

### Первичная настройка
```bash
# 1. Запустить миграции
python -m db.migrations.run_all_migrations

# 2. Проверить базу
python check_db.py

# 3. Заполнить настройки бота
python fill_bot_settings.py
```

### Проверка уведомлений
```bash
# 1. Создать тестовые данные
python setup_test_notifications.py --email your@email.com

# 2. Проверить что данные созданы
python check_db.py

# 3. Запустить планировщики
python -m scheduler.birthday_checker
python -m scheduler.booking_reminder_checker
```

### Обновление настроек бота
```bash
# 1. Проверить текущее состояние
python fill_bot_settings.py --check

# 2. Заполнить пустые поля
python fill_bot_settings.py

# 3. Или перезаписать все
python fill_bot_settings.py --force
```

---

## ⚠️ Важные замечания

1. **База данных:** Все скрипты работают с `salon_bot.db` в текущей директории
2. **Email:** Для тестирования уведомлений используйте реальный email адрес
3. **Время:** Тестовые события создаются на указанное количество дней вперед
4. **Безопасность:** При перезаписи настроек бота требуется подтверждение (кроме --force)

---

## 📚 Дополнительная документация

- `BOT_SETTINGS_README.md` - подробное описание fill_bot_settings.py
- `db/migrations/README.md` - документация по миграциям

---

## 🐛 Troubleshooting

### Ошибка: "Database not found"
```bash
# Убедитесь что находитесь в директории backend
cd /path/to/beauty-crm/backend
```

### Пустые поля бота после миграций
```bash
# Заполните их вручную
python fill_bot_settings.py
```

### Не приходят уведомления
```bash
# 1. Проверьте тестовые данные
python check_db.py

# 2. Убедитесь что email правильный
python setup_test_notifications.py --email correct@email.com

# 3. Проверьте настройки SMTP в .env файле
```
