...
# Instagram recipient_id для тестирования
# ВАЖНО: Замените на реальный Instagram ID получателя
TEST_INSTAGRAM_ID = "123456789"  # ← ЗАМЕНИТЕ НА РЕАЛЬНЫЙ ID
...
async def test_instagram_notification():
    ...
    if TEST_INSTAGRAM_ID == "123456789":
        print("\n⚠️  ПРОПУЩЕНО: Не указан реальный Instagram ID")
        print("\n💡 Чтобы протестировать Instagram:")
        print("   1. Откройте файл test_notifications_full.py")
        print("   2. Замените TEST_INSTAGRAM_ID на реальный Instagram ID")
        print("   3. Запустите тест еще раз")
        print("\n📝 Как получить Instagram ID:")
        print("   - Отправьте сообщение боту в Instagram")
        print("   - Проверьте логи сервера - там будет показан sender_id")
        return None
