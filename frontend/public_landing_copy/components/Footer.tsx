      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
          ...
          <div>
            ...
            <p className="text-primary-foreground/70 text-xs sm:text-sm leading-relaxed">
              {t('footerDesc', { defaultValue: "Премиальный салон красоты в центре города. Создаем безупречные образы с 2015 года." })}
            </p>
          </div>

          {/* Contact Info */}
          <div>
            <h4 className="text-base sm:text-lg mb-4">{t('contacts', { defaultValue: "Контакты" })}</h4>
          ...
          {/* Working Hours */}
          <div>
            <h4 className="text-base sm:text-lg mb-4">{t('workingHours', { defaultValue: "Часы работы" })}</h4>
            <div className="space-y-2 text-xs sm:text-sm text-primary-foreground/70">
              <p>{t('monSun', { defaultValue: "Понедельник - Воскресенье" })}</p>
              <p className="text-base sm:text-lg text-primary-foreground">{salonInfo?.hours_weekdays || "10:30 - 21:30"}</p>
              <p className="mt-4 text-primary-foreground/90">{t('noWeekends', { defaultValue: "Без выходных" })}</p>
            </div>
          </div>

          {/* Social Media */}
          <div>
            <h4 className="text-base sm:text-lg mb-4">{t('socialNetworks', { defaultValue: "Социальные сети" })}</h4>
        ...
        <div className="border-t border-primary-foreground/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
          <p className="text-xs sm:text-sm text-primary-foreground/70">
            © {currentYear} {salonInfo?.name || "Beauty Salon"}. {t('allRightsReserved', { defaultValue: "Все права защищены." })}
          </p>
