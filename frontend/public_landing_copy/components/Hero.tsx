          ...
          <div className="space-y-6 sm:space-y-8">
            <div className="space-y-4">
              <p className="text-xs sm:text-sm tracking-[0.2em] uppercase text-muted-foreground animate-fade-in">
                {t('heroTag', { defaultValue: "Премиальный салон красоты" })}
              </p>
              <h1 className="text-4xl sm:text-5xl lg:text-7xl tracking-tight text-primary animate-fade-in-up">
                {heroBanner ? getTranslatedText(heroBanner.title_ru, heroBanner.title_en, heroBanner.title_ar) : (t('heroTitle', { defaultValue: "Ваша красота —" }))}
                <br />
                <span className="text-accent-foreground">
                  {heroBanner ? getTranslatedText(heroBanner.subtitle_ru, heroBanner.subtitle_en, heroBanner.subtitle_ar) : (t('heroSubtitle', { defaultValue: "наша страсть" }))}
                </span>
              </h1>
            </div>

            <p className="text-base sm:text-lg text-foreground/70 max-w-xl animate-fade-in-up">
              {t('heroDescription', { defaultValue: "Откройте для себя мир изысканной красоты в атмосфере роскоши и комфорта. Профессиональный уход и безупречный сервис." })}
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 animate-fade-in-up">
              ...
              <Button
                ...
                size="lg"
              >
                {t('ourServices', { defaultValue: "Наши услуги" })}
              </Button>
            </div>
            ...
            <div className="flex flex-wrap gap-6 sm:gap-8 pt-4 sm:pt-8 border-t border-border/50 animate-fade-in">
              <div className="flex flex-col">
                ...
                <span className="text-xs sm:text-sm text-muted-foreground">{t('yearsExperience', { defaultValue: 'лет опыта' })}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-2xl sm:text-3xl text-primary">5000+</span>
                <span className="text-xs sm:text-sm text-muted-foreground">{t('happyClients', { defaultValue: 'довольных клиентов' })}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-2xl sm:text-3xl text-primary">100%</span>
                <span className="text-xs sm:text-sm text-muted-foreground">{t('qualityGuarantee', { defaultValue: 'гарантия качества' })}</span>
              </div>
            </div>
          </div>
        </div>
