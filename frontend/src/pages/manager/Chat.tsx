          ...
          selectedClient ? (
              ...
              < div className="bg-white border-b border-gray-200 px-6 py-4" >
                <div className="flex items-center justify-between" >
                  <div className="flex items-center gap-4" >
                    ...
                    < div >
                      <span className="text-sm text-gray-900 block" > {selectedClient.display_name} </span>
                      < span className="text-xs text-gray-500" >
                        {selectedClient.phone || 'В сети'}
                      </span>
                    </div>
                  </div>
                    ...
                    loadingMessages ? (
                      ...
                      messages.map((msg, index) => (
                        <div
                          ...
                          className={`flex ${(msg.sender === 'bot' || msg.sender === 'manager') ? 'justify-end' : 'justify-start'}`}
                        >
                          <div
                            className={`
                              ...
                              msg.type === 'image' ? (
                                <>
                                  <img
                                    ...
                                    alt="Изображение"
                                    loading="lazy"
                                    className="w-full h-auto max-h-72 object-cover cursor-pointer hover:opacity-90 transition-opacity rounded-xl mb-2"
                                    onClick={() => window.open(msg.message, '_blank')}
                                  />
                                  < div className={`flex items-center gap-1 ${(msg.sender === 'bot' || msg.sender === 'manager') ? 'justify-end' : 'justify-start'}`}>
                                    <span className={`text-xs ${(msg.sender === 'bot' || msg.sender === 'manager') ? 'text-white/80' : 'text-gray-500'}`}>
                                      {
                                ...
                                <>
                                  <video
                                    src={msg.message}
                                    controls
                                    className="w-full h-auto rounded-xl max-h-72 mb-2"
                                  />
                                  <div className={`flex items-center gap-1 ${(msg.sender === 'bot' || msg.sender === 'manager') ? 'justify-end' : 'justify-start'}`}>
                                    <span className={`text-xs ${(msg.sender === 'bot' || msg.sender === 'manager') ? 'text-white/80' : 'text-gray-500'}`}>
                                      {
                                ...
                                <>
                                  <a
                                    ...
                                    className={`flex items-center gap-2 hover:underline mb-2 ${(msg.sender === 'bot' || msg.sender === 'manager') ? 'text-white' : 'text-blue-600'
                                      }`}
                                  >
                                ...
                                <>
                                  ...
                                  < div className={`flex items-center gap-1 mt-1 ${(msg.sender === 'bot' || msg.sender === 'manager') ? 'justify-end' : 'justify-start'}`}>
                                    <span className={`text-xs ${(msg.sender === 'bot' || msg.sender === 'manager') ? 'text-white/80' : 'text-gray-500'}`}>
                                      {
                                        new Date(msg.timestamp).toLocaleTimeString('ru-RU', {
                                          hour: '2-digit',
                  ...
                  <div className="flex items-end gap-2" >
                    ...
                    <div className="flex-1 relative" >
                      <textarea
                        ...
                        rows={1}
                        disabled={isUploadingFile}
                        className="w-full px-4 py-2 text-sm bg-gray-100 border-0 rounded-full resize-none focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
                      />
                    </div>
                    ...
                    < button
                      ...
                      ${canSend && !isUploadingFile
                          ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white hover:shadow-lg'
                          : 'bg-gray-200 text-gray-400 cursor-not-allowed'
                        }
                    `}
                    >
                      {isUploadingFile ? <Loader size={20} className="animate-spin" /> : <Send size={20} />}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 flex items-center justify-center text-gray-400" >
              <div className="text-center" >
                ...
                < span className="text-sm text-gray-600 mb-2 block" > Выберите чат </span>
                < span className="text-sm text-gray-500" > Выберите диалог из списка слева </span>
              </div>
            </div>
          )}
      </div>
