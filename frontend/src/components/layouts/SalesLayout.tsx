...
export default function SalesLayout({ user, onLogout }: SalesLayoutProps) {
  ...
  const chatSubmenuItems = enabledMessengers.map(messenger => ({
    ...
    path: messenger.type === 'instagram' ? '/sales/chat' : `/sales/chat?messenger=${messenger.type}`,
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <span className="text-xs text-gray-500">{t('sales_panel', 'Отдел продаж')}</span>
              </div>
            </div>
          </div>
                      ...
                      {showChatSubmenu && (
                        <ul className="mt-1 ml-8 space-y-1">
                          {chatSubmenuItems.map((subItem, subIndex) => {
                            const isActive = location.pathname === '/sales/chat' &&
                              ((subItem.path.includes('?') && location.search.includes(subItem.path.split('?')[1])) ||
                                (subItem.path === '/sales/chat' && !location.search));
          ...
          <div className="p-4 border-t border-gray-200">
            <div className="flex items-center gap-3 mb-3">
              {userProfile?.photo ? (
                ...
                <img
                  src={getDynamicAvatar(userProfile?.full_name || user?.full_name || 'Sales', 'warm', 'female')}
                  alt={userProfile?.full_name}
                  className="w-10 h-10 rounded-full object-cover border-2 border-purple-100 shadow-sm"
                />
              )}
              <div className="flex-1 overflow-hidden">
                <span className="text-sm font-semibold text-gray-900 block truncate">{userProfile?.full_name || user?.full_name || 'Продавец'}</span>
                <div className="flex flex-col">
                  <span className="text-[10px] text-gray-500 capitalize leading-tight">{user?.role || 'sales'}</span>
                </div>
              </div>
            </div>
