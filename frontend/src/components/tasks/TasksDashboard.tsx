...
export function TasksDashboard({ tasks, stages }: TasksDashboardProps) {
    ...
    return (
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 h-full overflow-y-auto">
                ...
                <Card className="h-[400px] flex flex-col">
                    <CardHeader>
                        <CardTitle className="text-lg font-bold flex items-center gap-2">
                            ...
                            <Badge variant="secondary" className="ml-auto">{stats.today.length}</Badge>
                        </CardTitle>
                    </CardHeader>
                            ...
                            {stats.today.length > 0 ? (
                                <div className="space-y-3">
                                    {stats.today.map(task => (
                                        <div key={task.id} className="flex justify-between items-center py-2 border-b border-gray-50 last:border-0 hover:bg-gray-50 px-2 rounded-md transition-colors cursor-pointer">
                ...
                <Card className="h-[400px] flex flex-col">
                    <CardHeader>
                        <CardTitle className="text-lg font-bold flex items-center gap-2">
                            ...
                            <Badge variant="secondary" className="ml-auto">{stats.execution.length}</Badge>
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="flex-1 overflow-hidden p-0">
                        <ScrollArea className="h-full px-6 pb-4">
                            {stats.execution.length > 0 ? (
                                <div className="space-y-3">
                                    {stats.execution.map(task => (
                                        <div key={task.id} className="py-2 border-b border-gray-50 last:border-0 hover:bg-gray-50 px-2 rounded-md transition-colors cursor-pointer">
                        ...
                        {stats.overdueChartData.length > 0 ? (
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    ...
                                    <Tooltip />
                                    <Legend verticalAlign="bottom" height={36} />
                                    <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" className="text-2xl font-bold fill-red-500">
                                        {stats.overdue.length}
                                    </text>
                                </PieChart>
                            </ResponsiveContainer>
                        ) : (
                            <div className="h-full flex items-center justify-center text-green-500 text-sm flex-col">
            ...
            <div className="space-y-6">
                <Card className="h-[400px] flex flex-col bg-gradient-to-br from-indigo-50 to-white">
                    <CardHeader>
                        <CardTitle className="text-lg font-bold flex items-center gap-2 text-indigo-900">
                            🌪️ {t('pipeline_overview', 'Воронка задач')}
                        </CardTitle>
                    </CardHeader>
                                    ...
                                    return (
                                        <div key={stage.id} className="relative group">
                                            ...
                                            <div className="h-4 bg-gray-100 rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full rounded-full ${stage.color || 'bg-blue-500'} bg-opacity-80 transition-all duration-500`}
                                                    style={{ width: `${width}%` }}
                                                />
                                            </div>
                                        </div>
                ...
                <Card className="h-[400px] flex flex-col">
                    ...
                    <CardContent className="flex items-center justify-center text-center h-full">
                        <div className="space-y-2">
                            <p className="text-3xl font-bold text-gray-900">0.00 AED</p>
                            <p className="text-sm text-gray-500">{t('finance_module_coming_soon', 'Модуль финансов в разработке')}</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
