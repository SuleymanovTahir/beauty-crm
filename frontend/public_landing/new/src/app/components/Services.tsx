...
export function Services() {
  ...
  const filteredServices = services.filter((service) => {
    ...
    const matchesSearch =
      (service.title_ru && service.title_ru.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (service.title_en && service.title_en.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
              ...
              >
                <div className="flex justify-between items-start mb-3 sm:mb-4">
                  <h3 className="text-lg sm:text-xl font-medium text-[var(--heading)] group-hover:text-primary transition-colors">
                    {service[`title_${i18n.language}` as keyof Service] || service.title_ru}
                  </h3>
        ...
        {displayCount < filteredServices.length && (
          <div className="text-center mt-8 sm:mt-12">
            ...
            >
              {t('loading', { defaultValue: 'Показать еще' })}
            </Button>
          </div>
        )}
      </div>
