    ...
    <section id="testimonials" className="py-16 sm:py-20 lg:py-24 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Carousel
          ...
          className="w-full max-w-7xl mx-auto px-0 sm:px-12"
        >
          <CarouselContent className="-ml-3 sm:-ml-4">
            {testimonials.map((testimonial, index) => (
              <CarouselItem key={testimonial.id || index} className="pl-3 sm:pl-4 basis-full sm:basis-1/2 lg:basis-1/3 xl:basis-1/4 h-auto">
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                  animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          {/* Desktop Arrows */}
          <CarouselPrevious className="hidden sm:flex absolute -left-12 top-1/2 -translate-y-1/2 h-12 w-12 bg-white text-primary border-primary/20 shadow-md hover:bg-primary hover:text-white hover:border-primary transition-all duration-300" />
          <CarouselNext className="hidden sm:flex absolute -right-12 top-1/2 -translate-y-1/2 h-12 w-12 bg-white text-primary border-primary/20 shadow-md hover:bg-primary hover:text-white hover:border-primary transition-all duration-300" />

          {/* Mobile Controls */}
          <div className="flex justify-center gap-6 mt-6 sm:hidden">
            <CarouselPrevious className="static translate-y-0 h-12 w-12 bg-white border-primary/20 text-primary shadow-md hover:bg-primary hover:text-white transition-all duration-300" />
            <CarouselNext className="static translate-y-0 h-12 w-12 bg-white border-primary/20 text-primary shadow-md hover:bg-primary hover:text-white transition-all duration-300" />
          </div>
        </Carousel>
      </div>
    </section>
