...
def _get_recording_entry(cursor, source: str, recording_id: int) -> Optional[Dict[str, Any]]:
    if source == "telephony":
        cursor.execute("""
            SELECT
                cl.id,
                ...
                COALESCE(cl.manual_manager_name, b.master) AS manual_manager_name,
                NULL AS sender_id,
                NULL AS receiver_id
            FROM call_logs cl
        ...
        return {
            ...
            "source": "telephony",
            "type": "telephony",
            "recording_file": row[1],
            "recording_url": row[2],
            "manager_name": row[3],
            "manual_manager_name": row[4],
            "sender_id": None,
            "receiver_id": None,
        }
    ...
    return {
        ...
        "recording_url": row[2],
        "manager_name": None,
        "manual_manager_name": None,
        "sender_id": row[3],
        "receiver_id": row[4],
    }


def _require_recording_access(cursor, source: str, recording_id: int, current_user: Dict[str, Any]) -> Dict[str, Any]:
    if source not in {"telephony", "chat"}:
        raise HTTPException(status_code=400, detail="Invalid source")

    recording = _get_recording_entry(cursor, source, recording_id)
    if not recording:
        raise HTTPException(status_code=404, detail="Запись не найдена")

    if not check_recording_access(recording, current_user):
        raise HTTPException(status_code=403, detail="Нет прав для доступа к этой записи")

    return recording


def _resolve_local_recording_path(recording: Dict[str, Any]) -> Optional[str]:
    recording_file = str(recording.get("recording_file") or "").strip()
    if recording_file:
        file_path = os.path.join(RECORDINGS_DIR, recording_file)
        if os.path.exists(file_path):
            return file_path

    recording_url = str(recording.get("recording_url") or "").strip()
    if recording_url.startswith("/static/recordings/"):
...
async def _stream_remote_recording(recording_url: str):
...
@router.get("/recordings/{source}/{recording_id}/media")
async def get_recording_media(
    source: str,
    recording_id: int,
    current_user: dict = Depends(get_current_user)
):
    """Безопасная выдача записи только после проверки доступа."""
    conn = get_db_connection()
    c = conn.cursor()

    try:
        recording = _require_recording_access(c, source, recording_id, current_user)
        local_path = _resolve_local_recording_path(recording)
        if local_path:
        ...
        remote_url = str(recording.get("recording_url") or "").strip()
        if not remote_url.startswith(("http://", "https://")):
            raise HTTPException(status_code=404, detail="Файл записи недоступен")

        filename = str(recording.get("recording_file") or f"{source}_{recording_id}")
        return StreamingResponse(
            _stream_remote_recording(remote_url),
            media_type="audio/webm",
        ...
        logger.error(f"Error streaming remote recording {source}/{recording_id}: {error}")
        raise HTTPException(status_code=502, detail="Не удалось получить удаленную запись")
    except Exception as error:
        logger.error(f"Error serving recording media {source}/{recording_id}: {error}")
...
async def update_recording(
    ...
    try:
        ...
        _require_recording_access(c, source, recording_id, current_user)
