...
const Contracts = () => {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const [contractsResult, typesResult, stagesResult] = await Promise.allSettled([
            ...
            if (typesResult.status === 'fulfilled') {
                setContractTypes(typesResult.value.types || []);
            } else {
                console.error('Error loading contract types:', typesResult.reason);
            }
            if (stagesResult.status === 'fulfilled') {
                setContractStages(stagesResult.value || []);
            } else {
                console.error('Error loading contract stages:', stagesResult.reason);
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="crm-page crm-calendar-theme crm-calendar-page crm-calendar-contracts p-0 bg-gray-50/50 flex flex-col h-full overflow-hidden">
