...
export default function Cashbox() {
  ...
  const paymentMethodLabels: Record<string,string> = {
    cash: t('cashbox_payment_cash', { defaultValue: 'Наличные' }),
    card: t('cashbox_payment_card', { defaultValue: 'Карта' }),
    transfer: t('cashbox_payment_transfer', { defaultValue: 'Перевод' }),
    online: t('cashbox_payment_online', { defaultValue: 'Онлайн' }),
  };
  ...
  const save = async () => {
    if (!form.amount) { toast.error(t('cashbox_amount_required', { defaultValue: 'Укажите сумму' })); return; }
    ...
    if (d.success) { toast.success(t('cashbox_operation_saved', { defaultValue: 'Операция записана' })); setShowForm(false); load(); }
  ...
  const typeFilterOptions = [
    { value: '', label: t('cashbox_filter_all', { defaultValue: 'Все' }) },
    { value: 'income', label: t('cashbox_type_income_plural', { defaultValue: 'Доходы' }) },
    { value: 'expense', label: t('cashbox_type_expense_plural', { defaultValue: 'Расходы' }) },
  ];
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
          <p className="text-xs text-green-600 font-medium">{t('cashbox_type_income_plural', { defaultValue: 'Доходы' })}</p>
          <p className="text-2xl font-bold text-green-700 dark:text-green-400">{totals.income?.toLocaleString()} ₽</p>
        </div>
        <div className="bg-red-50 dark:bg-red-900/20 rounded-xl p-4 border border-red-200 dark:border-red-800">
          <p className="text-xs text-red-600 font-medium">{t('cashbox_type_expense_plural', { defaultValue: 'Расходы' })}</p>
        ...
        <div className={`${totals.profit >= 0 ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800' : 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800'} rounded-xl p-4 border`}>
          <p className={`text-xs font-medium ${totals.profit >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>{t('cashbox_profit', { defaultValue: 'Прибыль' })}</p>
      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 space-y-3">
          <h3 className="font-semibold">{t('cashbox_new_operation', { defaultValue: 'Новая операция' })}</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div>
              <label className="text-xs text-gray-500 mb-1 block">{t('cashbox_type', { defaultValue: 'Тип' })}</label>
              <select className="w-full border rounded-md px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600"
                ...
                <option value="income">{t('cashbox_type_income', { defaultValue: 'Доход' })}</option>
                <option value="expense">{t('cashbox_type_expense', { defaultValue: 'Расход' })}</option>
              </select>
            </div>
            <div>
              <label className="text-xs text-gray-500 mb-1 block">{t('cashbox_category', { defaultValue: 'Категория' })}</label>
              <select className="w-full border rounded-md px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600"
                value={form.category} onChange={e => setForm(p => ({...p,category:e.target.value}))}>
                {catOptions.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div><label className="text-xs text-gray-500 mb-1 block">{t('crm/users:amount')} *</label>
              <Input type="number" value={form.amount} onChange={e => setForm(p => ({...p,amount:e.target.value}))} /></div>
            <div>
              <label className="text-xs text-gray-500 mb-1 block">{t('cashbox_payment_method', { defaultValue: 'Способ оплаты' })}</label>
              <select className="w-full border rounded-md px-3 py-2 text-sm dark:bg-gray-800 dark:border-gray-600"
            ...
            <div><label className="text-xs text-gray-500 mb-1 block">{t('crm/settings:date')}</label>
              <Input type="date" value={form.operation_date} onChange={e => setForm(p => ({...p,operation_date:e.target.value}))} /></div>
            <div><label className="text-xs text-gray-500 mb-1 block">{t('description')}</label>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-900/50 text-xs text-gray-500">
              <tr>{[
                t('crm/settings:date'),
                t('cashbox_type', { defaultValue: 'Тип' }),
                t('cashbox_category', { defaultValue: 'Категория' }),
                t('crm/users:amount'),
                t('cashbox_payment_short', { defaultValue: 'Способ' }),
                t('description'),
                '',
                ...
                <tr key={op.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/30">
                  ...
                  <td className="px-4 py-3">
                    <span className={`flex items-center gap-1 font-medium ${op.type==='income'?'text-green-600':'text-red-600'}`}>
                      ...
                      {op.type==='income'
                        ? t('cashbox_type_income', { defaultValue: 'Доход' })
                        : t('cashbox_type_expense', { defaultValue: 'Расход' })}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-gray-500">{paymentMethodLabels[op.payment_method] ?? op.payment_method}</td>
                  <td className="px-4 py-3 text-gray-500 max-w-xs truncate">{op.description}</td>
                  <td className="px-4 py-3">
                    <button className="text-gray-400 hover:text-red-600" onClick={() => remove(op.id)} title={t('delete')}>×</button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {ops.length === 0 && <div className="text-center py-8 text-gray-400">{t('cashbox_empty', { defaultValue: 'Нет операций' })}</div>}
        </div>
      )}
    </div>
  );
