 ...
 */
import { buildApiUrl, buildWebSocketUrl } from '../api/client';
...
export class WebRTCService {
  ...
  private async connectWebSocket(): Promise<void> {
    return new Promise((resolve, reject) => {
      const wsUrl = buildWebSocketUrl('/api/webrtc/signal');
  ...
  async toggleDND(enabled: boolean): Promise<any> {
    const response = await fetch(buildApiUrl('/api/internal-chat/status/dnd?dnd=' + enabled), {
      method: 'POST',
  ...
  async getCallLogs(): Promise<any[]> {
    const response = await fetch(buildApiUrl('/api/internal-chat/call-logs'));
