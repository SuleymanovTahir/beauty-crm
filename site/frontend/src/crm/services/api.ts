...
export class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (import.meta.env.DEV && duration > ApiClient.longestRequest.duration) {
        ApiClient.longestRequest = { url: resolvedEndpoint, duration: duration }
      }
      ...
      if (resolvedEndpoint !== '/api/login') {
        ...
        if (import.meta.env.DEV) {
          if (isSlow) {
            console.warn(`⚠️ SLOW: ${resolvedEndpoint} (${durationFormatted})`)
          } else {
            console.log(`⏱️ ${resolvedEndpoint} (${durationFormatted})`)
          }
        } else if (isSlow) {
          console.warn(`⚠️ SLOW REQUEST: ${resolvedEndpoint} took ${durationFormatted}`)
        }
      }
  ...
  async getPlatformGates() {
    ...
    // Runtime profile is a fallback only when explicit product gates are absent.
    // In split deployment /api/runtime-profile can point to site runtime even on /crm frontend,
    // so it must not override explicit values from /api/platform-gates.
  ...
  async uploadRecording(callId: number, file: File) {
    ...
    const response = await fetch(buildApiUrl(`/api/telephony/upload-recording/${callId}`, this.baseURL), {
      method: 'POST',
      headers,
      credentials: 'include',
      body: formData
  ...
  async getRoles(): Promise<{ roles: Array<{ key: string; name: string; level: number }> }> {
    const response = await fetch(buildApiUrl('/api/roles', this.baseURL), {
      credentials: 'include'
    });
  ...
  async createUser(data: { username: string; password: string; full_name: string; email?: string; role: string }) {
    ...
    const response = await fetch(buildApiUrl('/api/register', this.baseURL), {
      method: 'POST',
      credentials: 'include',
      body: formData,
    })
  ...
  async exportClients(format: string = 'csv') {
    ...
    const response = await fetch(buildApiUrl(`/api/export/clients?format=${format}&lang=${lang}`, this.baseURL), {
      credentials: 'include',
    })
  ...
  async exportAnalytics(format: string = 'csv', period: number = 30, dateFrom?: string, dateTo?: string) {
    const lang = i18n.language || 'en';
    let endpoint = `/api/export/analytics?format=${format}&period=${period}&lang=${lang}`;
    if (dateFrom && dateTo) {
  ...
  async exportMessages(clientId?: string, format: string = 'csv', dateFrom?: string, dateTo?: string) {
    const lang = i18n.language || 'en';
    let endpoint = `/api/export/messages?format=${format}&lang=${lang}`;
  ...
  async exportFullData(format: string = 'csv') {
    const lang = i18n.language || 'en';
    const response = await fetch(buildApiUrl(`/api/export/full-data?format=${format}&lang=${lang}`, this.baseURL), {
      credentials: 'include',
    })
  ...
  async importClients(file: File) {
    ...
    const response = await fetch(buildApiUrl('/api/clients/import', this.baseURL), {
      method: 'POST',
      body: formData,
      credentials: 'include',
    })
  ...
  async uploadFile(file: File, subfolder?: string) {
    ...
    if (subfolder) {
      endpoint += `?subfolder=${encodeURIComponent(subfolder)}`
    }
    const url = buildApiUrl(endpoint, this.baseURL)
  ...
  async uploadRingtone(file: File) {
    ...
    const response = await fetch(buildApiUrl('/api/ringtones', this.baseURL), {
      method: 'POST',
      body: formData,
      credentials: 'include',
    })
