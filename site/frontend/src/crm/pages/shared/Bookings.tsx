...
const api = {
    ...
    async getBookings(params: any = {}) {
        ...
        const res = await fetch(buildApiUrl(`/api/bookings?${searchParams.toString()}`, this.baseURL), { credentials: 'include' });
        return res.json();
    },

    async getClients() {
        const res = await fetch(buildApiUrl('/api/clients', this.baseURL), { credentials: 'include' });
        return res.json();
    },
    ...
    async getServices(lang: string = 'ru', activeOnly: boolean = false) {
        ...
        const res = await fetch(buildApiUrl(`/api/services?${searchParams.toString()}`, this.baseURL), { credentials: 'include' });
        return res.json();
    },

    async updateBooking(id: number, data: any) {
        const res = await fetch(buildApiUrl(`/api/bookings/${id}`, this.baseURL), {
            method: 'PUT',
            credentials: 'include',
    ...
    async exportBookings(format: string, dateFrom?: string, dateTo?: string) {
        let endpoint = `/api/export/bookings?format=${format}`;
        if (dateFrom && dateTo) {
    ...
    async importBookings(file: File) {
        ...
        const res = await fetch(buildApiUrl('/api/import/bookings', this.baseURL), {
            method: 'POST',
            credentials: 'include',
            body: formData
        });
    ...
    async downloadImportTemplate(format: 'csv' | 'excel') {
        const res = await fetch(buildApiUrl(`/api/import/bookings/template?format=${format}`, this.baseURL), {
            credentials: 'include'
        });
    ...
    async getUsers(lang: string = 'ru') {
        const res = await fetch(buildApiUrl(`/api/users?language=${lang}`, this.baseURL), { credentials: 'include' });
        return res.json();
    },

    async createBooking(data: any) {
        const res = await fetch(buildApiUrl('/api/bookings', this.baseURL), {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data)
    ...
    async updateBookingStatus(id: number, status: string) {
        const res = await fetch(buildApiUrl(`/api/bookings/${id}/status`, this.baseURL), {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
    ...
    async getEmployeesForService(serviceId: number, lang: string = 'ru') {
        const res = await fetch(buildApiUrl(`/api/services/${serviceId}/employees?language=${lang}`, this.baseURL), {
            credentials: 'include'
        });
        return res.json();
    },
    ...
    async getEmployeeBusySlots(employeeId: number, date: string) {
        const res = await fetch(buildApiUrl(`/api/employees/${employeeId}/busy-slots?date=${date}`, this.baseURL), {
            credentials: 'include'
        });
        return res.json();
    }
