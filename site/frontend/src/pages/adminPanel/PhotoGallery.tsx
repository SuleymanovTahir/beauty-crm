...
export default function PhotoGallery() {
  ...
  const loadCategories = async () => {
    try {
      const response = await fetch(buildApiUrl('/api/admin/gallery/categories'), {
        credentials: 'include',
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const loadPhotos = async () => {
    try {
      setLoading(true);
      const response = await fetch(buildApiUrl('/api/admin/gallery/photos'), {
        credentials: 'include',
      });
  ...
  const loadStats = async () => {
    try {
      const response = await fetch(buildApiUrl('/api/admin/gallery/stats'), {
        credentials: 'include',
      });
  ...
  const handleDeletePhoto = async (id: string) => {
    ...
    try {
      const response = await fetch(buildApiUrl(`/api/admin/gallery/photos/${id}`), {
        method: 'DELETE',
        credentials: 'include',
      });
  ...
  const handleToggleFeatured = async (id: string, currentStatus: boolean) => {
    try {
      const response = await fetch(buildApiUrl(`/api/admin/gallery/photos/${id}/featured`), {
        method: 'PUT',
  ...
  const handleToggleVisibility = async (id: string, currentStatus: boolean) => {
    try {
      const response = await fetch(buildApiUrl(`/api/admin/gallery/photos/${id}/visibility`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
