"""
Дополнительные endpoints для client_auth.py
Добавить в файл client_auth.py после register_client endpoint
"""
...
async def resend_client_verification(email: str):
    ...
    try:
        ...
        c.execute("SELECT instagram_id, name, email FROM clients WHERE LOWER(email) = LOWER(%s)", (email,))
        client = c.fetchone()
