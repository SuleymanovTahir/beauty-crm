# 🚀 Инструкция по деплою Beauty CRM

## Быстрый деплой (автоматический)

### Вариант 1: Использовать скрипт

```bash
cd /Users/tahir/Desktop/beauty-crm/site
./deploy.sh
```

Скрипт автоматически:

1. ✅ Синхронизирует файлы backend и frontend на сервер
2. ✅ Перезапускает beauty_crm service
3. ✅ Перезапускает nginx
4. ✅ Показывает статус сервисов

### Вариант 2: Ручной деплой (пошагово)

#### Шаг 1: Синхронизация backend

```bash
rsync -avz --progress \
    --exclude='venv/' \
    --exclude='__pycache__/' \
    --exclude='*.pyc' \
    ./backend/ ubuntu@32.235:/home/ubuntu/beauty_crm/site/backend/
```

#### Шаг 2: Синхронизация frontend

```bash
rsync -avz --progress \
    --exclude='node_modules/' \
    --exclude='dist/' \
    ./frontend/ ubuntu@32.235:/home/ubuntu/beauty_crm/site/frontend/
```

#### Шаг 3: Перезапуск сервисов на сервере

```bash
ssh ubuntu@32.235
sudo systemctl daemon-reload
sudo systemctl restart beauty_crm
sudo systemctl restart nginx
```

## Проверка статуса

### Просмотр логов в реальном времени

```bash
ssh ubuntu@32.235 'sudo journalctl -u beauty_crm -f'
```

Или сразу после деплоя:

```bash
./deploy.sh && ssh ubuntu@32.235 'sudo journalctl -u beauty_crm -f'
```

### Статус сервиса

```bash
ssh ubuntu@32.235 'sudo systemctl status beauty_crm'
```

### Последние ошибки

```bash
ssh ubuntu@32.235 'sudo journalctl -u beauty_crm -n 50 --no-pager'
```

## Что было исправлено в этом деплое

### 1. Система записей (Recordings)

- ✅ Backend API полностью реализован ([backend/api/recordings.py](backend/api/recordings.py))
- ✅ Миграция базы данных ([backend/db/init.py](backend/db/init.py))
- ✅ 9 новых frontend компонентов в `frontend/src/components/recordings/`
- ✅ Вкладка "Записи" в телефонии
- ✅ Меню троеточия во внутреннем чате

### 2. Исправлены тесты

- ✅ Создана утилита [backend/tests/test_utils.py](backend/tests/test_utils.py)
- ✅ Исправлены duplicate key errors в тестах:
  - `test_detailed.py`
  - `test_new_features.py`
  - `test_schedule.py`
- ✅ Автоматическая очистка связанных данных (расписание, выходные, перерывы)

### 3. Исправлены импорты

- ✅ [backend/main.py:512](backend/main.py#L512) - `diagnostic_full` импорт
- ✅ [backend/main.py:41](backend/main.py#L41) - `middleware` импорт

### 4. Миграции

- ✅ Идемпотентная миграция для системы записей
- ✅ Автоматический запуск при старте сервера ([backend/main.py:598](backend/main.py#L598))

## Проверка после деплоя

### 1. Проверить запуск сервиса

```bash
ssh ubuntu@32.235 'sudo systemctl status beauty_crm'
```

Должно быть: `Active: active (running)`

### 2. Проверить тесты

Тесты теперь не должны падать с ошибками duplicate key:

```bash
ssh ubuntu@32.235 'cd /home/ubuntu/beauty_crm/site/backend && python3 tests/test_new_features.py'
```

### 3. Проверить систему записей

1. Откройте: `https://your-domain.com/admin/telephony`
2. Перейдите на вкладку "Записи"
3. Должна отобразиться система управления папками

### 4. Проверить внутренний чат

1. Откройте внутренний чат
2. Справа в заголовке должна быть кнопка с троеточием
3. Нажмите - должно открыться меню с тремя опциями

## Откат изменений (если что-то пошло не так)

### Быстрый откат

```bash
ssh ubuntu@32.235
cd /home/ubuntu/beauty_crm
git stash  # Сохранить текущие изменения
git pull origin main  # Получить последнюю версию
sudo systemctl restart beauty_crm
```

### Откат миграций

Если миграции вызвали проблемы, закомментируйте в `main.py`:

```python
# Строка 598
# run_all_migrations()  # Временно отключить
```

Затем:

```bash
sudo systemctl restart beauty_crm
```

## Структура деплоя

```
beauty_crm/
├── backend/
│   ├── main.py                           # Раскомментирован run_all_migrations()
│   ├── api/
│   │   └── recordings.py                 # Новый API для записей
│   ├── db/
│   │   └── migrations/consolidated/
│   │       └── schema_recording_folders.py  # Новая миграция
│   └── tests/
│       ├── test_utils.py                 # Новая утилита
│       ├── test_detailed.py              # Исправлен
│       ├── test_new_features.py          # Исправлен
│       └── test_schedule.py              # Исправлен
└── frontend/
    └── src/
        ├── components/
        │   ├── recordings/               # 9 новых компонентов
        │   └── shared/
        │       └── InternalChatWithWebRTC.tsx  # Добавлено меню
        ├── pages/admin/
        │   └── Telephony.tsx             # Добавлена вкладка
        └── locales/ru/admin/
            └── telephony.json            # 39 новых ключей
```

## Мониторинг после деплоя

### Следить за логами 5 минут после деплоя

```bash
ssh ubuntu@32.235 'sudo journalctl -u beauty_crm -f'
```

Следите за:

- ❌ Ошибки импорта (`ModuleNotFoundError`)
- ❌ Ошибки миграций (`duplicate key`, `table already exists`)
- ❌ Ошибки тестов (если они запускаются автоматически)
- ✅ Успешный запуск: `Application startup complete`

### Проверка использования памяти

```bash
ssh ubuntu@32.235 'free -h && ps aux | grep gunicorn | head -5'
```

## Полезные команды

### Просмотр конфигурации сервиса

```bash
ssh ubuntu@32.235 'sudo systemctl cat beauty_crm'
```

### Перезагрузка конфигурации без перезапуска

```bash
ssh ubuntu@32.235 'sudo systemctl reload beauty_crm'
```

### Проверка nginx конфигурации

```bash
ssh ubuntu@32.235 'sudo nginx -t'
```

## Контакты для поддержки

Если возникли проблемы:

1. Проверьте логи: `sudo journalctl -u beauty_crm -n 100`
2. Проверьте статус: `sudo systemctl status beauty_crm`
3. Проверьте документацию: `RECORDINGS_SETUP.md`, `TESTS_FIXED.md`

---

**Создано:** 11.01.2026
**Версия:** 1.0
**Автор:** Claude Code Assistant
